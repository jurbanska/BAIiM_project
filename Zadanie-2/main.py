...
def searchAPI(item):
    g.db = connect_db()
    #curs = g.db.execute("SELECT * FROM shop_items WHERE name=?", item) # Bezpieczna forma wyciągania
...
if __name__ == "__main__":
    if not os.path.exists(app.database):
        with sqlite3.connect(app.database) as connection:
            ...
            c.execute('INSERT INTO shop_items VALUES("woda", "40", "100")')
            c.execute('INSERT INTO shop_items VALUES("sok", "40", "110")')
            c.execute('INSERT INTO shop_items VALUES("chleb", "20", "10")')
            c.execute('INSERT INTO shop_items VALUES("cukierki", "200", "2")')
            c.execute('INSERT INTO employees VALUES("json", "{}")'.format(hash_pass("12345")))
            c.execute('INSERT INTO employees VALUES("admin", "{}")'.format(hash_pass("admin")))
            c.execute('INSERT INTO employees VALUES("robert", "{}")'.format(hash_pass("D0bR3_H4$l0_123")))
            connection.commit()
            connection.close()
